...
create table if not exists public.questions (
  ...
  max_attempts integer not null default 1 check (max_attempts between 1 and 99),
...
create table if not exists public.submissions (
  ...
  answer_elapsed_ms integer check (answer_elapsed_ms >= 0),
  final_status text check (final_status in ('correct', 'timeout', 'attempt_limit_exceeded')),
  attempt_count integer not null default 1 check (attempt_count >= 0),
  max_attempts_snapshot integer check (max_attempts_snapshot between 1 and 99),
  final_answer text,
  answered_before_reveal boolean not null default false,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'questions_max_attempts_check'
  ) then
    alter table public.questions
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'questions_time_limit_ms_check'
  ) then
    alter table public.questions
      add constraint questions_time_limit_ms_check check (time_limit_ms > 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'submissions_answer_elapsed_ms_check'
  ) then
    alter table public.submissions
      add constraint submissions_answer_elapsed_ms_check check (answer_elapsed_ms is null or answer_elapsed_ms >= 0);
  end if;

  if not exists (
    select 1
    from pg_constraint
    where conname = 'submissions_final_status_check'
  ) then
    alter table public.submissions
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'submissions_attempt_count_check'
  ) then
    alter table public.submissions
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'submissions_max_attempts_snapshot_check'
  ) then
    alter table public.submissions
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'submissions_answered_before_reveal_check'
  ) then
    alter table public.submissions
