...
結果発表の表示方法は `/admin` で選べます。「画面共有用の結果発表を開く」は管理者用の別タブを開き、参加者画面は通常表示を維持します。「参加者画面を結果発表に切り替える」は参加済みの全員の `/play` を結果表示に切り替えます（約3秒以内）。「参加者画面を通常表示に戻す」で問題画面に戻せます。通常時の参加者向け結果APIは非公開です。結果表示中は参加者側の解答操作を表示しませんが、開始済み問題のタイマーは継続するため、解答を締め切ってから切り替えてください。

`/admin` と管理者用結果ページでログイン情報が無効、またはイベントの操作権限がない場合は `/admin/login` に移動します。イベント所有者のアカウントでログインしてください。
