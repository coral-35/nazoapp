...
export async function loadResults(roomId: string, questionsPerSet: number, participantId?: string) {
  ...
  for (let offset = 0; ; offset += 1000) {
    const { data, error } = await db.from("questions").select("id, order_index").eq("room_id", roomId).order("id").range(offset, offset + 999);
    if (error) throw new Error("問題集計の取得に失敗しました。");
    questions.push(...data);
    if (data.length < 1000) break;
  }
  for (let offset = 0; ; offset += 1000) {
    let query = db.from("submissions").select("participant_id, question_id, is_correct, answer_elapsed_ms").eq("room_id", roomId).eq("is_correct", true).order("id").range(offset, offset + 999);
