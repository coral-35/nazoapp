...
const { data: existing, error: lookupError } = await db.from("rooms").select("id, title, created_by, room_code").eq("id", sample.roomId).maybeSingle();
check(lookupError);
if (existing && (existing.title !== SAMPLE_TITLE || existing.created_by !== userId)) {
  throw new Error("サンプル用IDが別のルームで使用されています。処理を中止しました。");
}
let roomCode = existing?.room_code;
if (!existing) {
  for (let attempt = 0; attempt < 10; attempt++) {
    roomCode = String(randomInt(1000000)).padStart(6, "0");
    const { error } = await db.from("rooms").insert({ id: sample.roomId, room_code: roomCode, title: SAMPLE_TITLE, created_by: userId, questions_per_set: 7, status: "finished" });
    if (!error) break;
...
check((await db.from("rooms").update({ questions_per_set: 7, status: "finished", current_question_id: null }).eq("id", sample.roomId)).error);
for (const [table, expected] of [["participants", 50], ["questions", 21], ["submissions", 1050]]) {
  const { count, error } = await db.from(table).select("id", { count: "exact", head: true }).eq("room_id", sample.roomId);
  check(error);
  if (count !== expected) throw new Error(`${table}: 件数が想定と異なります。`);
}
console.log(`サンプル作成完了: 50人 / 21問 / 1050解答 / 3セット\nルーム番号: ${roomCode}\n結果発表: /admin/rooms/${sample.roomId}/results`);
