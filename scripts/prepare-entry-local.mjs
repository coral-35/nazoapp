...
if (defaultRoomId() !== SAMPLE_DEFAULT_ROOM_ID) throw new Error("サンプル以外の既定イベントは変更しません。");
const db = createClient(config.supabaseUrl, config.serviceRoleKey, { auth: { persistSession: false, autoRefreshToken: false } });
const { data: room, error } = await db.from("event_settings").select("id, title, status").eq("id", SAMPLE_DEFAULT_ROOM_ID).single();
if (error || !room) throw new Error("先に npm run seed:results:local でサンプルを作成してください。");
if (room.title !== SAMPLE_TITLE) throw new Error("想定するサンプルイベントと一致しません。");
if (room.status === "finished" || room.status === "draft") {
  const { error: updateError } = await db.from("event_settings").update({ status: "waiting", current_question_id: null }).eq("id", room.id);
